    ...
    return AppBackground(
      child: SafeArea(
        child: Scaffold(
          ...
          appBar: AppBar(
            backgroundColor: Colors.transparent,
            title: const CustomText(text: 'الإعلانات'),
            centerTitle: true,
          ),
          floatingActionButton: FloatingActionButton.extended(
            ...
            label: const CustomText(text: 'إعلان جديد'),
          ),
          body: BlocConsumer<AlertCubit, AlertState>(
            listener: (ctx, state) {
              final c = ctx.read<AlertCubit>();
            ...
            builder: (ctx, state) {
              ...
              if (state is AlertErrorState && _ads.isEmpty) {
                return Center(
                  child: CustomText(
                    text: state.message,
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                CustomText(
                                  text:
                                      'من: ${ad.createdAt.day}/${ad.createdAt.month}/${ad.createdAt.year}',
                                ),
                                const Spacer(),
                                CustomText(
                                  text:
                                      'إلى: ${ad.expiresAt.day}/${ad.expiresAt.month}/${ad.expiresAt.year}',
                                  color: isExpired
                                      ? AppColors.snackError
                                      : AppColors.gold,
                                ),
